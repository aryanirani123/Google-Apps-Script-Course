function getYoutube(){
  ...
  Logger.log("The title has been added");

  var final_stats = stats_vids.items.map(function(res){ return [res.statistics.viewCount,res.statistics.likeCount,res.statistics.dislikeCount,res.statistics.commentCount]});
  //Logger.log(final_stats);

  sheet.getRange(2,3, final_stats.length,final_stats[0].length).setValues(final_stats);
  Logger.log("The stats have been added");
